...
class MessageReceiver:
        ...
        if self.receiver.threadId is not None:
            ...
            if thread is not None:
                ...
                if self.receiver.imageMessageId2 is not None:
                    self.image_message_2 = await self.channel.fetch_message(
    ...
    async def create_thread(self):
        ...
        second_image = await self.channel.send("** **")
...
async def on_clan_battle_guide_publish():
    ...
    job.guides.sort(key=lambda g: g.damage.lstrip("0").zfill(5), reverse=True)
    ...
    for boss_index, boss in enumerate(job.clanBattle.bosses):
        ...
        team_sheet_2 = await get_team_sheet(job, boss, 7)
...
def build_timeline_content(
    ...
    timelines: list[str] = []
    for guide in guides:
        timeline = guide.timeline.strip()
        if timeline == "":
            continue

        team_content = ""

        for member in guide.teamMembers[::-1]:
            team_content += f"{member.star}★{member.character.name['zh-Hant']}"

            if member.remark != "":
                remark = member.remark.replace("\n", " ")
                team_content += f"　※{remark}"

            team_content += "\n"

        in_scope = False
        lines: list[str] = [team_content]
